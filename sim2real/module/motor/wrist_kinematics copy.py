...
kps = np.array([180, 220, 120, 180, 120, 120, 180, 220, 120, 180, 120, 120], dtype=np.double)*0.3
kds = np.array([ 10, 8, 8, 10, 5, 5, 10,8, 8, 10, 5, 5,], dtype=np.double)*0.6
...
if __name__ == "__main__":
    ...
    # 计算第五个和第六个关节电机的角度
    motor5_angle_rad, motor6_angle_rad = AnkleJointModule.ankle_joint_ik(pitch_rad, roll_rad)
    motor5_angle_deg = np.rad2deg(motor5_angle_rad)
    motor6_angle_deg = np.rad2deg(motor6_angle_rad)
    print("motor5_angle_deg:", motor5_angle_deg)
    print("motor6_angle_deg:", motor6_angle_deg)

    # 假设前四个电机角度为0，后两个电机使用计算得到的角度
    target_position = [0, 0, 0, 0, motor5_angle_deg, motor6_angle_deg, 0, 0, 0, 0, 0, 0]    
