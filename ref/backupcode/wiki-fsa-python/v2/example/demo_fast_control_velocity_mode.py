...
def main():
    ...
    if server_ip_list:
        ...
        for i in range(len(server_ip_list)):
            fi_fsa_v2.fast_set_velocity_control(server_ip_list[i], 0.0)
        ...
        for i in range(len(server_ip_list)):
            fi_fsa_v2.fast_set_mode_of_operation(
                server_ip_list[i], fi_fsa_v2.FSAModeOfOperation.VELOCITY_CONTROL
            )
        ...
        for t in range(count_max):
            for i in range(len(server_ip_list)):
                ...
                fi_fsa_v2.fast_set_velocity_control(server_ip_list[i], set_velocity)
            time.sleep(0.01)
