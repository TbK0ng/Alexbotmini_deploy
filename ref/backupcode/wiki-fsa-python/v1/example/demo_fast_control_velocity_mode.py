...
def main():
    ...
    if server_ip_list:
        ...
        for i in range(len(server_ip_list)):
            fi_fsa_v1.fast_set_velocity_control(server_ip_list[i], 0.0)
        ...
        for i in range(len(server_ip_list)):
            fi_fsa_v1.fast_set_enable(server_ip_list[i])
        ...
        for i in range(len(server_ip_list)):
            fi_fsa_v1.fast_set_mode_of_operation(
                server_ip_list[i], fi_fsa_v1.FSAModeOfOperation.VELOCITY_CONTROL
            )
        ...
        for t in range(count_max):
            for i in range(len(server_ip_list)):
                set_velocity = 600.0 * math.sin(t / 1000.0)  # [deg/s]
                fi_fsa_v1.fast_set_velocity_control(server_ip_list[i], set_velocity)
            time.sleep(0.01)
        ...
        for i in range(len(server_ip_list)):
            fi_fsa_v1.fast_set_disable(server_ip_list[i])
