...
class robot_config:
    ...
    tau_limit = np.array([80, 40, 40, 80, 20, 20, 80, 40, 40, 80, 20, 20], dtype=np.double)
    ...
    def run_alexbotmini(policy, cfg):
        ...
        while(True):
            ...
            if count_lowlevel % 10 == 0:
                ...
                motor.set_position_control(target_q*180/math.pi)
            ...
            # data.ctrl = tau
            # sleep 10ms
            target_qtest = target_q * 180 / math.pi
            print('target_q = :',target_qtest)# degree
            tau = np.clip(tau, -robot_config.tau_limit, robot_config.tau_limit)
            print('tau = :',tau)
            count_lowlevel += 1   
