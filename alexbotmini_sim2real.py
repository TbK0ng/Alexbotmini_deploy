...
class robot:
    ...
    def get_obs():
        motor.get_pvc()
        # 直接获取最新的IMU数据，不再依赖n相关逻辑
        quat, gvec = imu.cmd_read(port, baudrate)
        q = motor.q
        dq = motor.dq
        print("Quat value type:", type(quat))  
        print("quat value:", quat)
        print("gvec value:", gvec)
        print("Quat value shape:", quat.shape)  
    ...
    def run_alexbotmini(policy, cfg):
        ...
        while(True):
            ...
            # Generate PD control
            tau = utils.pd_control(target_q, q, robot_config.kps,
                            target_dq, dq, robot_config.kds)  # Calc torques
